...
namespace gr
{
  namespace ais
  {
    ...
    {
      return gnuradio::get_initial_sptr(new pdu_to_nmea_impl(designator));
    }
    ...
    std::string pdu_to_nmea_impl::to_sentence(std::string ascii, uint8_t npad)
    {
      ...
      while (frag_id <= num_frags)
      {
        if (frag_id > 1)
          ret += "\n";
        std::string this_sentence = "!AIVDM," + boost::to_string(num_frags) + "," + boost::to_string(frag_id++) + ",," + d_designator + ",";
